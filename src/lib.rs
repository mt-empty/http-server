    ...
    fn parse_headers(headers: Vec<(&str, &str)>) -> Vec<Header> {
        headers
            ...
            .filter_map(|(key, value)| {
                SupportedHeader::from_str(key.trim().to_lowercase().as_str())
                    .ok()
                    .map(|key| Header {
                        key,
                        value: Rc::new(value.trim().to_string()),
                    })
            })
            .collect::<Vec<Header>>()
    }
...
pub fn start_server(directory: Option<String>) {
    let directory = directory.unwrap_or_else(|| "/tmp/".to_string());
